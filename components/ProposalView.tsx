  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Proposals & Contracts</h1>
          <p className="text-zinc-500 mt-1">Create winning proposals with AI-assisted drafting.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-3xl">
            <label className="block text-xs font-bold text-zinc-500 uppercase mb-3">Select Deal</label>
            <select
            ...
            >
              <option value="">Choose an open deal...</option>
              {deals.map(deal => (
            ...
            >
              ...
              {drafting ? 'Drafting...' : 'AI Generate Draft'}
            </button>
          </div>

          <div className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-3xl space-y-4">
            <h3 className="font-bold text-sm border-b border-zinc-800 pb-3">Draft Actions</h3>
            <button className="w-full flex items-center justify-between px-4 py-2 hover:bg-zinc-800 rounded-lg text-sm transition-colors text-zinc-400 hover:text-white">
              <span className="flex items-center gap-3"><Eye size={16} /> Preview Mode</span>
            </button>
            <button className="w-full flex items-center justify-between px-4 py-2 hover:bg-zinc-800 rounded-lg text-sm transition-colors text-zinc-400 hover:text-white">
              <span className="flex items-center gap-3"><Send size={16} /> Send to Client</span>
            </button>
          </div>
        </div>
          ...
          <div className="flex-1 p-12 bg-white text-zinc-900 rounded-b-3xl overflow-y-auto no-scrollbar font-serif">
            {proposalContent ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-zinc-300 gap-4 opacity-50">
                ...
                <p className="font-sans text-sm italic">Select a deal and click generate to start your proposal draft</p>
              </div>
            )}
          </div>
        </div>
